...
// Admin UI pages — auth-gated so non-admins can't even fetch the HTML
...
app.get('/admin/monthly-goal', (req, res, next) => {
  const { getSession } = require('./lib/sessions');
  const session = getSession(req.cookies?.kellis_session);
  if (!session || session.role !== 'admin') return res.redirect('/login.html');
  next();
}, (_req, res) => res.sendFile(path.resolve(__dirname, '../admin/monthly-goal.html')));
